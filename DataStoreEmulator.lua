...
function DatastoreService:GetDataStore(name)
    local success, result = retryRequest(function()
        ...
        return HttpService:PostAsync(BACKEND_URL .. "/createdatastore", json, Enum.HttpContentType.ApplicationJson)
    end)
    processQueue()
...
function DataStore:GetAsync(key)
    local success, result = retryRequest(function()
        ...
        local url = BACKEND_URL .. "/getdata?key=" .. HttpService:UrlEncode(self.StoreName .. "_" .. key)
...
function DataStore:SetAsync(key, value)
    local success, result = retryRequest(function()
        ...
        local json = HttpService:JSONEncode(payload)
        return HttpService:PostAsync(BACKEND_URL .. "/setdata", json, Enum.HttpContentType.ApplicationJson)
    end)
    processQueue()
    if success then
        return true
...
function DataStore:RemoveAsync(key)
    local success, result = retryRequest(function()
        if not throttleRequest() then return false end
        local url = BACKEND_URL .. "/removedata?key=" .. HttpService:UrlEncode(self.StoreName .. "_" .. key)
        return HttpService:GetAsync(url)
    end)
    processQueue()
    if success then
